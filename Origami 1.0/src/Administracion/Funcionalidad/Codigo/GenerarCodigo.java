...
public class GenerarCodigo implements Serializable {
    ...
    /**
     * en lugar de setInstrucccion() llamar a crear la instancia
     * setCodeOfFigure() generateCodeC() || generateCodeCpp() getCode()
     */
    ...
    public void setInstrucciones(Vector<String> codigo,
	    Vector<String> tablaVariables) {
	this.codeOfFigure = new Vector<String>();
	this.TableOfVariable = new Vector<String>();
    ...
    /*
     * public String getCodigoC(){ generarCodigoC(); return code; } public
     * String getCodigoCmasmas(){ generarCodigoCmasmas(); return code; }
     */
    ...
    /*
     * private void generarCodigoCmasmas(){ leerDatos(false);
     * imprimirDatos(false); codigoTotal =
     * ("#include <iostream>\n"+"using namespace std;\n"
     * +"int main(int argc, char argv[])\n{\n"); for(int
     * x=0;x<codigo.size();x++){ String str = codigo.elementAt(x); int pos =
     * str.indexOf("null"); if(pos<0){ codigoTotal = codigoTotal +
     * codigo.elementAt(x) + "\n"; } } codigoTotal = codigoTotal +
     * "      return 0;\n" + "}\n"; } private void generarCodigoC(){
     * leerDatos(true); imprimirDatos(true); codigoTotal =
     * ("#include <stdio.h>\n" + "#include <stdlib.h>"
     * +"\nint main(int argc, char argv[])\n{\n"); for(int
     * x=0;x<codigo.size();x++){ String str = codigo.elementAt(x); int pos =
     * str.indexOf("null"); if(pos<0){ codigoTotal = codigoTotal +
     ...
     */
    /*
     * private void leerDatos(boolean esC){ int j; for(int
     * x=0;x<codeOfFigure.size();x++){
     * if(codeOfFigure.elementAt(x).lastIndexOf("Leer:")>=0){ Vector<String>
     * lineas =leerDatosMultiples(codeOfFigure.elementAt(x), esC); String tab =
     * codeOfFigure.elementAt(x); j = codeOfFigure.elementAt(x).indexOf("L");
     * tab = tab.substring(0,j); removerLineasRepetidas(lineas);
     * codeOfFigure.removeElementAt(x); for(int k=0;k<lineas.size();k++){ String
     * lin = tab + lineas.elementAt(k); codeOfFigure.insertElementAt(lin,x);
     * x++; } x--; } } }
     */
    ...
    /*
     * public void removerLineasRepetidas(Vector<String> lineas){ String
     * aux,aux2,aux3; int pos; for(int i=0; i<lineas.size(); i++){ aux =
     * lineas.elementAt(i); while(aux.startsWith(" ")){ aux = aux.substring(1);
     * } pos = aux.indexOf(" "); while(pos!=-1){ aux3 = aux.substring(pos+1);
     * aux = aux.substring(0,pos)+ aux3; pos = aux.indexOf(" "); } for(int j=0;
     * j<codeOfFigure.size(); j++){ aux2 = codeOfFigure.elementAt(j);
     * while(aux2.startsWith(" ")){ aux2 = aux2.substring(1); } pos =
     * aux2.indexOf(" "); while(pos!=-1){ aux3 = aux2.substring(pos+1); aux2 =
     * aux2.substring(0,pos)+ aux3; pos = aux2.indexOf(" "); }
     * if(aux.compareTo(aux2)==0){ if(aux.indexOf("cin>>")==-1 &&
     * aux.indexOf("scanf(")==-1){ lineas.remove( i); i--; break; } } } } }
     */
    /**
     ...
     * Vector <String> datos = null; Vector <String> variables = new
     * Vector<String>(); String scanCaracter; String scanEnteros; String
     * scanFlotante; String scanDefaul; String ultimo; if(esC){ scanCaracter =
     * "scanf(" + "\"" + "%" +"c"+ "\"" + "," +"&"; scanEnteros = "scanf("
     * +"\""+"%"+"d"+"\""+","+"&"; scanFlotante = "scanf("
     * +"\""+"%"+"f"+"\""+","+"&"; scanDefaul = "scanf(" + "\"" + "%" +"d"+ "\""
     * + "," +"&"; ultimo = ");"; } else{ scanCaracter = "cin>>"; scanEnteros =
     * "cin>>"; scanFlotante = "cin>>"; scanDefaul = "cin>>"; ultimo = ";"; }
     * int i = linea.indexOf(","); if(i!=-1){ datos = new Vector<String>();
     * linea = linea.substring(linea.lastIndexOf("Leer:")+5,linea.length());
     * variables = separarVariables(linea); for(int k=0; k<variables.size();
     * k++){ datos.add(variables.elementAt(k)); } for(int k=0;
     * k<variables.size(); k++){
     * if(variables.elementAt(k).lastIndexOf("int")>=0){ String dato =
     * variables.
     * elementAt(k).substring(variables.elementAt(k).lastIndexOf("int")
     * +3,variables.elementAt(k).lastIndexOf(";")); String lin = scanEnteros +
     * dato + ultimo; datos.add(lin); } else
     * if(variables.elementAt(k).lastIndexOf("char")>=0){ String dato =
     * variables
     * .elementAt(k).substring(variables.elementAt(k).lastIndexOf("char"
     * )+4,variables.elementAt(k).lastIndexOf(";")); String lin = scanCaracter +
     * dato + ultimo; datos.add(lin); } else
     * if(variables.elementAt(k).lastIndexOf("float")>=0){ String dato =
     * variables
     * .elementAt(k).substring(variables.elementAt(k).lastIndexOf("float"
     * )+5,variables.elementAt(k).lastIndexOf(";")); String lin = scanFlotante +
     * dato + ultimo; datos.add(lin); } else{ String dato =
     * variables.elementAt(k
     * ).substring(0,variables.elementAt(k).lastIndexOf(";")); String lin =
     * scanDefaul + dato + ultimo; datos.add(lin); } } } else{ datos = new
     * Vector<String>(); if(linea.lastIndexOf("int")!=-1){ String dato =
     * linea.substring(linea.lastIndexOf("int"),linea.length()-1);
     * TableOfVariable.add(dato +";"); String lin = scanEnteros +
     * dato.substring(3) + ultimo; datos.add(dato +";"); datos.add(lin); } else
     * if(linea.lastIndexOf("char")!=-1){ String dato =
     * linea.substring(linea.lastIndexOf("char"),linea.length()-1);
     * TableOfVariable.add(dato+";"); String lin2 = scanCaracter +
     * dato.substring(4) + ultimo; datos.add(dato+";"); datos.add(lin2); } else
     * if(linea.lastIndexOf("float")!=-1){ String dato =
     * linea.substring(linea.lastIndexOf("float"),linea.length()-1);
     * TableOfVariable.add(dato+";"); String lin2 = scanFlotante +
     * dato.substring(5) + ultimo; datos.add(dato+";"); datos.add(lin2); } else{
     * String tipo =
     * tipoDeDato(linea.substring(linea.lastIndexOf("Leer:")+5,linea
     * .length()-1)); if(tipo!=null){ if(tipo.lastIndexOf("int")>=0){
     * datos.add(scanEnteros +
     * linea.substring(linea.lastIndexOf("Leer:")+5,linea.length()-1)+ ultimo);
     * } else if(tipo.lastIndexOf("char")>=0){ datos.add(scanCaracter +
     * linea.substring(linea.lastIndexOf("Leer:")+5,linea.length()-1)+ ultimo);
     * } else{ datos.add(scanFlotante +
     * linea.substring(linea.lastIndexOf("Leer:")+5,linea.length()-1)+ ultimo);
     * } } else{ String lin2 = scanDefaul +
     * linea.substring(linea.lastIndexOf("Leer:")+5,linea.length()-1)+ ultimo;
     * datos.add(lin2); } } } return datos; }
     */
    ...
    /**
     * Este metodo separa las multiples variables para de una linea en la cual
     * se maneja el comando leer:
     * 
     * @param linea
     * @return Vector<String>
     * 
    ...
    /*
     * public void imprimirDatos(boolean esC){ Vector <String> imprimir = new
     * Vector<String>(); String printEnteros; String printFlotantes; String
     * printCaracteres; String printTexto; String printDefaul; String print;
     * String entero; String caracter; String flotante; String separador; String
     * ultimo; if(esC){ printEnteros = "printf("+"\""+"%d"+"\""+",";
     * printFlotantes = "printf("+"\""+"%f"+"\""+","; printCaracteres =
     * "printf("+"\""+"%c"+"\""+","; printTexto = "printf("+"\""; printDefaul =
     * "printf("+"\""+"%d"+"\""+","; print = "printf("+"\""; entero = "%d";
     * caracter = "%c"; flotante = "%f"; separador = ","; ultimo = ");"; } else{
     * printEnteros = "cout <<"; printFlotantes = "cout <<"; printCaracteres =
     * "cout <<"; printTexto = "cout <<"+"\""; printDefaul = "cout <<"; print =
     * "cout <<"; entero = ""; caracter = ""; flotante = ""; separador = "<<";
     * ultimo = "<< endl;"; } for(int x=0;x<codeOfFigure.size();x++){ String aux
     * = codeOfFigure.elementAt(x); while(aux.startsWith(" ")){ aux =
     * aux.substring(1); } if(aux.startsWith("\\p")){ String tab =
     * codeOfFigure.elementAt
     * (x).substring(0,codeOfFigure.elementAt(x).indexOf("\\"));
     * if(aux.indexOf(",")>0){ String inicio; String fin=""; if(esC){ fin=","; }
     * String aux2; int comaAnterior = aux.indexOf(","); int comaPosterior =
     * aux.indexOf(",",comaAnterior+1); aux2 = aux.substring(0,comaAnterior);
     * while(aux2.endsWith(" ")){ aux2 = aux2.substring(0,aux2.length()-1); }
     * if(aux2.startsWith("\\p"+"\"") && aux2.endsWith("\"")){ inicio =
     * aux2.substring(aux.indexOf("\"")+1,aux2.length()-1); if(!esC){ inicio="";
     * fin = fin +aux2.substring(2)+separador; } } else{ String tipo =
     * getTypeOfData(aux2.substring(2)); if(tipo!=null){
     * if(tipo.indexOf("int")==0){ inicio = entero; fin = fin +
     * aux2.substring(2)+ separador; } else if(tipo.indexOf("char")==0){ inicio
     * = caracter; fin = fin + aux2.substring(2)+ separador; } else{ inicio =
     * flotante; fin = fin + aux2.substring(2)+ separador; } } else{ inicio =
     * entero; fin = fin + aux2.substring(2)+ separador; } }
     * while(comaPosterior!=-1){ aux2 = aux.substring(comaAnterior+1,
     * comaPosterior); while(aux2.startsWith(" ")){ aux2 = aux2.substring(1); }
     * while(aux2.endsWith(" ")){ aux2 = aux2.substring(0,aux2.length()-1); }
     * if(aux2.startsWith("\"") && aux2.endsWith("\"")){ inicio = inicio + " "
     * +aux2.substring(1,aux2.length()-1); if(!esC){ inicio=""; fin = fin
     * +aux2+separador; } } else{ String tipo = getTypeOfData(aux2);
     * if(tipo!=null){ if(tipo.indexOf("int")==0){ inicio = inicio +" "+ entero;
     * fin = fin +aux2+ separador; } else if(tipo.indexOf("char")==0){ inicio =
     * inicio +" "+ caracter; fin = fin +aux2+ separador; } else{ inicio =
     * inicio +" "+ flotante; fin = fin +aux2+ separador; } } else{ inicio =
     * inicio +" "+ entero; fin = fin +aux2+ separador; } } comaAnterior=
     * comaPosterior; comaPosterior = aux.indexOf(",",comaAnterior+1); } aux2 =
     * aux.substring(comaAnterior+1); while(aux2.startsWith(" ")){ aux2 =
     * aux2.substring(1); } if(aux2.endsWith(";")){ aux2 =
     * aux2.substring(0,aux2.length()-1); } if(aux2.startsWith("\"") &&
     * aux2.endsWith("\"")){ inicio = inicio + " "
     * +aux2.substring(1,aux2.length()-1); if(!esC){ inicio=""; fin = fin +aux2;
     * } } else{ String tipo = getTypeOfData(aux2); if(tipo!=null){
     * if(tipo.indexOf("int")==0){ inicio = inicio +" "+ entero; fin = fin +
     * aux2; } else if(tipo.indexOf("char")==0){ inicio = inicio +" "+ caracter;
     * fin = fin + aux2; } else{ inicio = inicio +" "+ flotante; fin = fin +
     * aux2; } } else{ inicio = inicio +" "+ entero; fin = fin + aux2; } }
     * while(fin.endsWith(",")){ fin = fin.substring(0,fin.length()-1); }
     * if(esC){ inicio = print+ inicio + "\""+ fin +");"; inicio = tab + inicio;
     * codeOfFigure.removeElementAt(x); codeOfFigure.insertElementAt(inicio,x);
     * imprimir.removeAllElements(); } else{ inicio=""; inicio = print+ inicio+
     * fin +"<< endl;"; inicio = tab + inicio; codeOfFigure.removeElementAt(x);
     * codeOfFigure.insertElementAt(inicio,x); imprimir.removeAllElements(); } }
     * else{ if(aux.startsWith("\\p"+"\"") && aux.endsWith("\""+";")){ int pos =
     * aux.indexOf("\""); int fin = aux.indexOf("\"", pos+1); String imp = tab +
     * printTexto + aux.substring(pos+1,fin)+"\""+ultimo; imprimir.add(imp);
     * codeOfFigure.removeElementAt(x);
     * codeOfFigure.insertElementAt(imprimir.elementAt(0),x);
     * imprimir.removeAllElements(); } else{ String tipo =
     * getTypeOfData(aux.substring(2)); if(tipo!=null){
     * if(tipo.indexOf("int")==0){ int pos = aux.indexOf("p"); int fin =
     * aux.indexOf(";"); if(fin!=-1){ String imp = tab + printEnteros +
     * aux.substring(pos+1,fin)+ultimo; imprimir.add(imp); } else{ String imp =
     * tab + printEnteros + aux.substring(pos+1,fin-1)+ultimo;
     * imprimir.add(imp); } } else if(tipo.indexOf("char")==0){ int pos =
     * aux.indexOf("p"); int fin = aux.indexOf(";"); if(fin!=-1){ String imp =
     * tab + printCaracteres + aux.substring(pos+1,fin)+ultimo;
     * imprimir.add(imp); } else{ String imp = tab + printCaracteres +
     * aux.substring(pos+1,fin-1)+ultimo; imprimir.add(imp); } } else { int pos
     * = aux.indexOf("p"); int fin = aux.indexOf(";"); if(fin!=-1){ String imp =
     * tab + printFlotantes + aux.substring(pos+1,fin)+ultimo;
     * imprimir.add(imp); } else{ String imp = tab + printFlotantes +
     * aux.substring(pos+1,fin-1)+ultimo; imprimir.add(imp); } }
     * codeOfFigure.removeElementAt(x);
     * codeOfFigure.insertElementAt(imprimir.elementAt(0),x);
     * imprimir.removeAllElements(); } else{ int pos = aux.indexOf("p"); int fin
     * = aux.indexOf(";"); if(fin!=-1){ String imp = tab + printDefaul +
     * aux.substring(pos+1,fin)+ultimo; imprimir.add(imp); } else{ String imp =
     * tab + printDefaul + aux.substring(pos+1,fin-1)+ultimo; imprimir.add(imp);
     * } codeOfFigure.removeElementAt(x);
     * codeOfFigure.insertElementAt(imprimir.elementAt(0),x);
    ...
    /*
     * public String tipoDeDato(String variable){ for(int i=0;
     * i<TableOfVariable.size(); i++){
     * if(TableOfVariable.elementAt(i).lastIndexOf(variable)>0){
     * if(TableOfVariable.elementAt(i).lastIndexOf("int")>=0){ return "int"; }
     * if(TableOfVariable.elementAt(i).lastIndexOf("char")>=0){ return "char"; }
     * if(TableOfVariable.elementAt(i).lastIndexOf("float")>=0){ return "float";
     * } } } return null; }
     */
	...
	for (int i = 0; i < lineas.length; i++) {
	    if (i > 3
		    && (!lineas[i].contains(impresion))
		    && (lineas[i].contains(entero)
			    || lineas[i].contains(flotante) || lineas[i]
			    .contains(caracter))) {
