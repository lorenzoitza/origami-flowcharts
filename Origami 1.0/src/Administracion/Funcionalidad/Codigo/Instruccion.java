 ...
 * TODO Extract Class de parte grafica
 * @version Origami 1.0
...
//	    ArreglarPasoAPaso();
	 ...
	 * @return int
	 */
	public int addFiguresInstructionToString(Vector<Figura> flowchartFigures,int indexStart){
	    int index;
	    
	    for (index = indexStart; index < flowchartFigures.size(); index++) {
		
		if (flowchartFigures.elementAt(index) instanceof DecisionFigure) {
		    DecisionFigure f = (DecisionFigure)flowchartFigures.elementAt(index);
			...
			if(f.instruction.instruccion.firstElement().getInstruccionSimple().compareTo("null") != 0 && f.instruction.instruccion.firstElement().getInstruccionSimple().compareTo("") != 0){
			    codigo.add(identador + f.instruction.instruccion.elementAt(0).getInstruccionSimple());
			    tabula(true);
			    index = addFiguresInstructionToString(flowchartFigures,index+2);
			    tabula(false);
			    codigo.add(identador+"}");
			    int y = codigo.size();
			    tabula(true);
			    index = addFiguresInstructionToString(flowchartFigures,index+2)+1;
			    tabula(false);
			    if(codigo.size()>y){
			...
			if(f.instruction.instruccion.firstElement().getInstruccionSimple().compareTo("null") != 0 && f.instruction.instruccion.firstElement().getInstruccionSimple().compareTo("") != 0){
			    codigo.add(identador + f.instruction.instruccion.elementAt(0).getInstruccionSimple());
			    tabula(true);
			    index = addFiguresInstructionToString(flowchartFigures,index+1);
			    tabula(false);
			    codigo.add(identador+"}");
			}
			else{
			...
			if(f.instruccion.instruccion.firstElement().getInstruccionSimple().compareTo("null") != 0 && f.instruccion.instruccion.firstElement().getInstruccionSimple().compareTo("") != 0){
			    codigo.add(identador + f.instruccion.instruccion.elementAt(0).getInstruccionSimple());
			    tabula(true);
			    index = addFiguresInstructionToString(flowchartFigures,index+1);
			    tabula(false);
			    codigo.add(identador+"}");
		    ...
		    OutputFigure figure = ((OutputFigure) flowchartFigures.elementAt(index));
		    ...
		    InputFigure f = ((InputFigure)flowchartFigures.elementAt(index));
			...
			for(int d=0;d<ins.length;d++){
				...
				for(int i=0; i<masVariables.size(); i++){
				    codigo.add(identador + masVariables.elementAt(i) + ";"); 
		    ...
		    SentenceFigure f = (SentenceFigure)flowchartFigures.elementAt(index);
		    if(f.instruccion.getInstruccionSimple() != null){
			codigo.add(identador + f.instruccion.getInstruccionSimple());
		    }
		}
	 ...
	 * @return int
	 */
	public int[] findFigureEnd(int cont, String controlFigure,String firstFigure,String secondFigure){
		...
		if(codigo.elementAt(x).indexOf(ciclo) != -1 && codigo.elementAt(x).indexOf(controlFigure) != -1){
		    inicio++;
		    if(cont==inicio){
			cont=0;
			inicio=1;
		...
		else if((codigo.elementAt(x).indexOf("else") != -1 || codigo.elementAt(x).indexOf(firstFigure) != -1 || codigo.elementAt(x).indexOf(secondFigure) != -1)&& bandera){
		    fin--;
		}
	    }
	    iniFin[1]=x-1;
