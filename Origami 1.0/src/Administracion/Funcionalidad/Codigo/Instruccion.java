...
public class Instruccion implements Serializable {
	...
	codigoCoCmasMas.generateCodeCpp();
	codigoTotal = codigoCoCmasMas.getCode();
	vectorCiclos(diagrama);
	codigoTotal = codigoCoCmasMas.getCodeGDB(codigoTotal);
	;
	// llamar al metodo que agregara a los if y while "&& VaRiAbLe==0"
	// para que asi lo cambie el codigoTotal antes de que se mande a
	// compilar y ejecutar.
	ArreglarPasoAPaso();
    }
	...
	if (comaPosterior == -1) {
		...
		if (comaPosterior != -1
		    ...
		    Vector<String> variables2 =
			    masVariables(linea.substring(comaAnterior + 1),
				    tipoAnterior);
	...
	for (x = 0; x < codigo.size(); x++) {
	    ...
	    if (codigo.elementAt(x).indexOf(ciclo) != -1
		    && codigo.elementAt(x).indexOf("if") != -1) {
		inicio++;
		if (cont == inicio) {
		    cont = 0;
		    inicio = 1;
		...
		fin++;
		    ...
		    || codigo.elementAt(x).indexOf("while") != -1 || codigo
		    .elementAt(x).indexOf("for") != -1)
		    && bandera) {
		fin--;
	    }
	}
    ...
    /**
     * Este metodo se encarga de encontrar el final de un ciclo For.
     * 
     * @param cont
     * @return int
     */
	...
	for (x = 0; x < codigo.size(); x++) {
	    ...
	    if (codigo.elementAt(x).indexOf(ciclo) != -1
		    && codigo.elementAt(x).indexOf("for") != -1) {
		inicio++;
		if (cont == inicio) {
		    cont = 0;
		    inicio = 1;
    ...
    /**
     * Este metodo se encarga de encontrar el final de un ciclo While.
     * 
     * @param cont
     * @return int
     */
	...
	for (x = 0; x < codigo.size(); x++) {
	    ...
	    if (codigo.elementAt(x).indexOf(ciclo) != -1
		    && codigo.elementAt(x).indexOf("while") != -1) {
		inicio++;
		if (cont == inicio) {
		    cont = 0;
		    inicio = 1;
		...
		fin++;
		    ...
		    || codigo.elementAt(x).indexOf("for") != -1 || codigo
		    .elementAt(x).indexOf("if") != -1)
		    && bandera) {
		fin--;
	    }
	...
	while (true) {
		...
		if (f.instruction.getInstruccionSimple() != null) {
			...
			if (coma != -1) {
			    Vector<String> masVariables =
				    masVariables(ins[d], null);
				...
				codigo.add(espacio + masVariables.elementAt(i)
					+ ";");
				int u =
					masVariables.elementAt(i).indexOf(
						"Leer:");
				if (u == -1) {
