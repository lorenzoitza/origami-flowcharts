...
public class EventoKey {
	...
	case 49:
		...
		Ventana.getComponentes().cursor[0] =
			new Cursor(Ventana.display, ImageLoader.getImage(
				"cursorEntrada.png").getImageData(), 0, 0);
	...
	case 50:
		...
		Ventana.getComponentes().cursor[0] =
			new Cursor(Ventana.display, ImageLoader.getImage(
				"cursorProceso.png").getImageData(), 0, 0);
	...
	case 52:
		...
		Ventana.getComponentes().cursor[0] =
			new Cursor(Ventana.display, ImageLoader.getImage(
				"cursorWhile.png").getImageData(), 0, 0);
	...
	case 53:
		...
		Ventana.getComponentes().cursor[0] =
			new Cursor(Ventana.display, ImageLoader.getImage(
				"cursorFor.png").getImageData(), 0, 0);
	...
	case 54:
		...
		Ventana.getComponentes().cursor[0] =
			new Cursor(Ventana.display, ImageLoader.getImage(
				"cursorSalida.png").getImageData(), 0, 0);
	...
	case 127:
		...
		if (tab.getHoja().getDiagrama().elementAt(
			selec.getFiguraSeleccionada()) instanceof CircleFigure) {
		} else {
		    EventoMenuContextual.Eliminar(tab.getHoja().getDiagrama()
			    .elementAt(selec.getFiguraSeleccionada()));
		}
	    }
	    break;
	case 16777226:
	...
	case 16777230:
	    if (!Ventana.getComponentes().isPasoAPaso) {
		CodeCompiler codigo = new CodeCompiler(Ventana.diagramas);
		...
		if (codigo.isError) {
		    ...
		    Ventana.componentes.text.setText(codigo.errorTipe);
		    tab.getTabItem().getInfo().addInformation(
	...
	if (key + key2 == 262241) {
	    if (Ventana.componentes.eje != null
		    && Ventana.componentes.getEnEjecucion()
		    && tab.getSelectedTabItemId() == Ventana.componentes.eje.a
			    .GetId()) {
		Ventana.componentes.stopEjecucion();
	    } else if (Ventana.componentes.paso != null
		    && Ventana.componentes.getEnEjecucion()
		    && tab.getSelectedTabItemId() == Ventana.componentes.paso.a
			    .GetId()) {
		Ventana.componentes.stopEjecucion();
	    }
		...
		if (file.exists()) {
			...
			tab.getTabItem().agregarRetroceso(
				tab.getHoja().getDiagrama(), selec);
		    }
		...
		if (archivo != null) {
			...
			MessageBox messageBox =
				new MessageBox(Ventana.shell, SWT.ICON_ERROR
					| SWT.OK);
	    ...
	    if (selec.getFiguraSeleccionada() != -1) {
		if (tab.getHoja().getFigureIndexOf(
			selec.getFiguraSeleccionada()) instanceof CircleFigure
			|| selec.getFiguraSeleccionada() == -1) {
		} else {
		    EventoMenuContextual.Cortar(tab.getHoja().getFigureIndexOf(
			    selec.getFiguraSeleccionada()));
		...
		if (tab.getHoja().getFigureIndexOf(
		    ...
		    EventoMenuContextual.Copiar(tab.getHoja().getFigureIndexOf(
			    selec.getFiguraSeleccionada()));
		    Ventana.getComponentes().toolBarDisable();
		}
	    }
		...
		if (Ventana.diagramaEnMemoria.diagrama.size() > 0) {
		    EventoMenuContextual.Pegar(tab.getHoja().getFigureIndexOf(
			    selec.getFiguraSeleccionada()));
		    Ventana.getComponentes().toolBarDisable();
		}
	    }
	    ...
	    /*
	     * //llevra a un txt la info FileDialog dialog = new
	     * FileDialog(Ventana.shell,SWT.SAVE);
	     * dialog.setFilterExtensions(new String[] { "*.txt","*.*"}); String
	     * archivo = dialog.open(); if(archivo!=null){ Exportar exp = new
	     * Exportar(tab); exp.exportarInfo(archivo); }
	     */
	}
    }
