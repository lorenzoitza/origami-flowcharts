...
public class AbstractSourceCode implements Serializable {
    ...
    private String getDataType(String previousDeclaration) {
	...
	if(line.lastIndexOf("Leer:")!=-1){
		...
		String previousTipe = getDataType(data);
		variables.add(lines[0]);
		for(int i=1; i<lines.length; i++){
		    String type = getDataType(lines[i]);
		    if(type!=""){
			previousTipe = type;
		...
		//declaraciones normales
		 String lines[]=line.split(",");
		 String previousTipe = getDataType(lines[0]);
		 variables.add(lines[0]);
		 for(int i=1; i<lines.length; i++){
		     String type = getDataType(lines[i]);
		     if(type!=""){
