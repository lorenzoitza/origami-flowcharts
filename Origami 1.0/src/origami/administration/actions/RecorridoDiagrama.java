...
public class RecorridoDiagrama {
    ...
    public static int recorridoCiclo(Vector<FigureStructure> diagrama,int i){
    	...
    	i++;
    	while(true){
    	    if(diagrama.elementAt(i) instanceof EllipseFigure && diagrama.elementAt(i).getBounds().x==x && diagrama.elementAt(i).getBounds().y==y){
    		break;
    	    }
    	    i++;
    	}
    ...
    public static int recorridoCiclo3(Vector<FigureStructure> diagrama,int i){
    	...
    	i++;
    	while(true){
    	    if(diagrama.elementAt(i) instanceof EllipseFigure && diagrama.elementAt(i).getBounds().x==x
    		    && diagrama.elementAt(i+1) instanceof EllipseFigure && diagrama.elementAt(i+2) instanceof EllipseFigure
