...
public class DatosDecision {
	...
	public DatosDecision(TabFolder tabfolder){
		tab = tabfolder;
	}
	/**
	 ...
	 * @param fig
	 */
	public void ventana(Display d,If fig,AdminSeleccion selec) {
	...
	public void BorrarCodigo(boolean mostrar){
		...
		if(mostrar){
			if(text.getText() != ""){
				...
				String instruc = "if(" + text.getText() + "){";
				...
				if(fig.instruccion.instruccion.size()>0){
					if(!fig.instruccion.instruccion.elementAt(0).instruccion.equals(instruc)){
						...
						tab.getTabItem().getInfo().setInformacion("/M - Se agrego o modifico una instruccion en una figura de tipo \"si\"\n");
						cambio=true;
					}
				}
