...
public class Ventana {
	...
	pasoApaso.addSelectionListener(new SelectionAdapter() {
		...
		if (!diagramas.getTabItem().getSave().isSave()) {
			...
			if (codigo.isError) {
			    ...
			    // moverConsola(true);
			    componentes.text.setText(codigo.errorTipe);
				...
				nombre =
					nombre
						.substring(0, nombre
							.indexOf("."));
				Exportar expor = new Exportar(diagramas);
				expor.exportarJpg(archivo, diagramas.getHoja()
					.getDiagrama(), diagramas.getHoja()
					.getConexion());
				break;
			    case 128:
				break;
	...
	diagramas.getHoja().getChart().disableCursor(
		diagramas.getHoja().getDiagrama(),
		diagramas.getHoja().getChart());
    }
	...
	try {
		...
		public void shellDeactivated(ShellEvent e) {
		    ...
		    diagramas.getHoja().getDibujarDiagrama().setCursor(
			    cursor[0]);
		    if (oldCursor != null) {
			oldCursor.dispose();
		    }
