 ...
 * dibujar a la figura del While.
 * @version Origami 1.0
 * @author Juan Ku, Victor Rodriguez
 */
public class While extends Figura{
	public int color;
	public InstruccionCompuesta instruccion = new InstruccionCompuesta();
	public While(int color){
	...
	public void paint (Graphics g){
		...
		if(instruccion.instruccion.size() >1 && instruccion.instruccion.firstElement().getInstruccionSimple().compareTo("null")!=0){
			if(instruccion.instruccion.firstElement().instruccion.length() <= 8){
				int i=1;
				...
				if(numero+2 <=10){
					String subStr = str.substring(6,numero);
				    g.drawText(subStr, r.x -16+r.width/2, r.y + 13);
				}
				else{
					int h = numero-9;
					String subStr = str.substring(6,numero - h);
				    g.drawText(subStr+"...", r.x -16+r.width/2, r.y + 13);
				}
			}
		}
